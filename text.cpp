...
void ttf_init()
{
    ...
    if (!stbtt_InitFont(&ttfi, file_buffer, 0))
    {
...
textured_quad ttf_create(char *text, int fsize, bool centerH, bool centerV, int bgA)
{
    ...
    int bitmapH = 0;
    int fontH = fsize;
    float scale = stbtt_ScaleForPixelHeight(&ttfi, fontH);

    int ascent, descent, lineGap;
    stbtt_GetFontVMetrics(&ttfi, &ascent,&descent,&lineGap);
    ...
    for (int i = 0; i < strlen(text); i++)
    {
        ...
        {
            // int x0,y0,x1,y1; stbtt_GetCodepointBitmapBox(&ttfi, text[i], scale,scale, &x0,&y0,&x1,&y1);
            int advance;    stbtt_GetCodepointHMetrics(&ttfi, text[i], &advance, 0);
            int kerning =   stbtt_GetCodepointKernAdvance(&ttfi, text[i], text[i+1]);
    ...
    for (int i = 0; i < strlen(text); i++)
    {
        ...
        {
            // int L,R,T,B;     stbtt_GetCodepointBitmapBox(&ttfi, text[i], scale,scale, &L,&T,&R,&B);
            int x0,y0,x1,y1; stbtt_GetCodepointBitmapBox(&ttfi, text[i], scale,scale, &x0,&y0,&x1,&y1);
            int advance;     stbtt_GetCodepointHMetrics(&ttfi, text[i], &advance, 0);
            int kerning =    stbtt_GetCodepointKernAdvance(&ttfi, text[i], text[i+1]);
            ...
            if (text[i] != ' ' && text[i] != NBSP)
                stbtt_MakeCodepointBitmapSubpixel(&ttfi, gray_bitmap+offset, x1-x0, y1-y0, bitmapW, scale,scale,x_shift,0, text[i]);
