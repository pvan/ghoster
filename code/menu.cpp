...
menuItem menuItems[] =
{
    ...
    {ID_FULLSCREEN   , L"Fullscreen"                     , &glass.is_fullscreen        , 0, 0, 0 },
    {ID_REPEAT       , L"Repeat"                         , &projector.state.repeat     , 0, 0, 0 },
    {ID_VOLUME       , L"Volume"                         , 0, &projector.state.volume  , 0, 0 },
    {ID_SEP          , L""                               , 0                           , 0, 0, 0 },
    ...
    {ID_SEP          , L""                               , 0                           , 0, 0, 0 },
    {ID_TOPMOST      , L"Always On Top"                  , &glass.is_topmost           , 0, 0, 0 },
    {ID_CLICKTHRU    , L"Ghost Mode (Cannot Be Clicked)" , &glass.is_clickthrough      , 0, 0, 0 },
    {ID_WALLPAPER    , L"Stick To Wallpaper"             , &glass.is_wallpaper         , 0, 0, 0 },
    {ID_TRANSPARENCY , L"Opacity"                        , 0, &glass.opacity           , 0, 0 },
    {ID_SEP          , L""                               , 0                           , 0, 0, 0 },
...
void onMenuItemClick(HWND hwnd, menuItem item)
{
    switch (item.code)
    {
        ...
        case ID_FULLSCREEN:
            ...
            break;
        case ID_SNAPPING:
            glass.set_snappy(!glass.is_snappy);
            break;
        case ID_WALLPAPER:
            glass.set_wallpaper(!glass.is_wallpaper);
            break;
        case ID_RANDO:
            queue_random_url();
            break;
    ...
    if (index >= 0)
    {
            ...
            || popupSliderCapture)  // also update if we're off the slider but dragging it around
        {
            ...
            *destination_value = result;
            ...
            if (destination_value == &glass.opacity)
                glass.set_opacity(result);
            if (destination_value == &projector.state.volume)
                projector.SetVolume(result);
    ...
    for (int i = 0; i < menuCount; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (i == 0 && !projector.state.is_paused
...
{
    menuItem *menu = menu_iconsubmenu;
    int menuCount = sizeof(menu_iconsubmenu) / sizeof(menuItem);
    ...
    switch(message)
    {
        ...
        case WM_PAINT: {
            PaintMenu(hwnd, menu, sizeof(menu_iconsubmenu)/sizeof(menuItem), subMenuSelectedItem);
            return 0;
        } break;
    }
...
LRESULT CALLBACK PopupWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch(message)
    {
        ...
        case WM_MOUSEMOVE: {
            ...
            int wid = SMI_WID;
            int hei = MI_HEI * (sizeof(menu_iconsubmenu)/sizeof(menuItem)) + 8;

            // lock submenu to monitor
            MONITORINFO mi = { sizeof(mi) };
            if (GetMonitorInfo(MonitorFromWindow(glass.hwnd, MONITOR_DEFAULTTOPRIMARY), &mi))
            {
